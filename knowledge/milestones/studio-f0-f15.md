---
type: milestone
status: verified
updated: 2026-09-18
tags:
  - studio
  - pyside6
---

# Studio F0–F15

| Faz | Kalıcı sonuç |
|---|---|
| F0–F1 | Hafıza politikası ve ölçümlü mevcut durum tespiti |
| F2 | Qt'siz token/tema katmanı ve Studio kabuğu |
| F3 | Processing 1.1.0 toplamsal backend ekleri |
| F4 | Proje, katılımcı, ayarlar ve giriş kapısı |
| F5 | Hafif capture önizlemesi ve ayrı kayıp sayaçları |
| F6 | Ayrı süreçte işleme, gerçek ilerleme, duraklat/iptal |
| F7 | İşlenen sürüm kütüphanesi, thumbnail ve karşılaştırma |
| F8–F9 | Kanonik annotation, timeline ve 2B/3B iskelet |
| F10 | Sporcu seçimi ve belirsiz aralık onayı |
| F11 | Dataset ve kanonik export kapıları |
| F12 | Altı ayrılabilir yardımcı pencere |
| F13 | Otomatik kayıt ve başarısız yazmada iş kaybını önleme |
| F14 | Klavye, kontrast, ölçek ve boş durum cilası |
| F15 | Uçtan uca tur ve bulunan iki gerçek hatanın onarımı |

## Sonraki saha kapanışları

- 16 Eylül: kayıt hedefi, gerçek ayar durumu, global kayıt kontrolü ve GUI/UX.
- 17 Eylül: yayımlama kullanılabilirliği, anchor, imza ısınması ve kalite ayrımı.
- 18 Eylül: gerçek ZED ile kişi seçili zincir ve veri kalitesi ölçümü.
- 19 Eylül: Studio GUI son iyileştirmesi, kendi 17 fazlık planıyla ayrı
  yürütüldü. Kapsam yalnız `studio/`; eski `gui/` dokunulmadı. Kalıcı
  sonuçlar: token şeması 1.2.0 ve paketlenmiş marka fontu; bildirim katmanı
  sayfa yığınından ayrıldı; Etiketleme'ye girişte native pencere yeniden
  oluşturma giderildi; 3B iskelet anatomik renk ve küre eklemlerle yeniden
  yazıldı; zemin düzlemi offline ölçülüyor (processing 1.2.0); hata sınıfı
  kendi eklemlerini taşıyor (canonical annotation 1.2.0, release 1.1.0);
  sağ panel kelime sekmesi yerine ikon şeridi.
  Ayrıntı: [faz planı](../plans/studio-gui-refinement-implementation.md) ·
  [doğrulama raporu](../reports/studio-gui-refinement-validation.md).

Tam faz kanıtı ve commitleri:
[faz planı](../archive/reports/FAZ_PLANI_PYSIDE6_STUDIO.md),
[Git kilometre taşları](git-history.md),
.

## Tarihsel kaynaklar

Bu notun dayandığı bölünmüş eski hafıza kayıtları. Tarihsel ayrıntı
gerekmedikçe açılmaz.

- [2. Mevcut aşama](../archive/memory/2-2-mevcut-asama.md)
- [6Y. PySide6 arayüz yeniden yapımı — F0 hafıza politikası + F1 tespiti (2026-09-13)](../archive/memory/6y-6y-pyside6-arayuz-yeniden-yapimi-f0-hafiza-politikasi-f1-tespiti-2026-09.md)
- [6Z. Studio arayüzü F2 — katman, token, kabuk (2026-09-13)](../archive/memory/6z-6z-studio-arayuzu-f2-katman-token-kabuk-2026-09-13.md)
- [6AA. Studio F3 — backend toplamsal ekleri, processing 1.1.0 (2026-09-13)](../archive/memory/6aa-6aa-studio-f3-backend-toplamsal-ekleri-processing-1-1-0-2026-09-13.md)
- [6AB. Studio F4–F7 — ekranlar, ayrı süreç, kütüphane (2026-09-13)](../archive/memory/6ab-6ab-studio-f4f7-ekranlar-ayri-surec-kutuphane-2026-09-13.md)
- [6AC. Studio F8 — Etiketleme, kanonik sidecar 1.1.0 (2026-09-14)](../archive/memory/6ac-6ac-studio-f8-etiketleme-kanonik-sidecar-1-1-0-2026-09-14.md)
- [6AD. Studio F9–F15 — 3B, sporcu, dışa aktarım, cila (2026-09-14)](../archive/memory/6ad-6ad-studio-f9f15-3b-sporcu-disa-aktarim-cila-2026-09-14.md)

Tam liste: [Tarihsel MEMORY arşivi](../archive/memory/index.md).
