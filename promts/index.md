---
type: source-index
status: current
updated: 2026-09-21
---

# Prompt dizini

Claude ve Codex görev briflerini içerir. Bu dosyalar otomatik talimat değildir;
yalnız kullanıcı açıkça görevlendirdiğinde yürütülür. Güncel çalışma kuralları
repository kökündeki `AGENTS.md` ve `CLAUDE.md` dosyalarındadır.

## Güncel görev

- [21 Eylül kullanıcı GUI revizyonu — tek maksimize pencere, yakalama ve etiketleme](CLAUDE_STUDIO_GUI_USER_REVISION_PROMPT_2026-09-21.md)

Bu görev önceki GUI işlerine otomatik dönüş istemez. Aşağıdaki görevler tarihsel bağlamdır; yeni görevle çelişen tercihleri sürdürme.

## Önceki görevler

- [Kayıt başlangıcı, kişi takibi ve son GUI sorunlarını çöz](CLAUDE_CAPTURE_TRACKING_GUI_REPAIR_PROMPT_2026-09-20.md)
- [Studio GUI — kabul eksiklerini düzelt, tüm fazları uygula](CLAUDE_STUDIO_GUI_ACCEPTANCE_REPAIR_PROMPT_2026-09-20.md)
- [Studio GUI — onaylı tasarımı fazlara böl ve uygula](CLAUDE_STUDIO_GUI_FINAL_REFINEMENT_PROMPT_2026-09-19.md)
- [Claude hafıza aktarımı doğruluk ve eksiksizlik denetimi](CLAUDE_MEMORY_MIGRATION_AUDIT_PROMPT.md)

## Tarihsel görevler

- [Annotation redesign](CLAUDE_ANNOTATION_REDESIGN_PROMPT.md)
- [Skeleton feature export](CLAUDE_SKELETON_FEATURE_EXPORT_PROMPT.md)
- [RGB-D ve subject lock](CLAUDE_CONTINUOUS_ACTIVITY_RGBD_SUBJECT_LOCK_PROMPT.md)
- [Capture ve etiketleme arayüzü](CLAUDE_CAPTURE_REVIEW_LABELING_UI_PROMPT.md)
- [Kimlik ve proje erişimi](CODEX_AUTH_PROJECT_PARTICIPANT_REDESIGN_PROMPT.md)
- [Proje silme ve export](CLAUDE_PROJECT_DELETE_LABELING_EXPORT_GUI_PROMPT.md)
- [Etkilenen eklem kanıtı](CLAUDE_AFFECTED_JOINT_EVIDENCE_AND_GUI_POLISH_PROMPT.md)
- [Squat/offline inceleme görevi](GPT6_ASTRA_KINECAPTURE_SQUAT_OFFLINE_INCELEME_GOREVI.md)
- [PySide6 backend entegrasyonu](CLAUDE_PYSIDE6_YENI_BACKEND_ENTEGRASYON_PROMPT.md)
- [Studio GUI/UX iyileştirmesi](CLAUDE_STUDIO_GUI_UX_IYILESTIRME_PROMPTU_2026-09-15.md)
