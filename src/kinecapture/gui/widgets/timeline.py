...
class TimelineMode(str, Enum):
    ...
    Exactly two, and they are the two levels of the repetition model. There was
    a third - a whole-take "activity" strip - and it is gone: it asked a
    different question from the other two, was authored through a different
    mental model, and in practice split the annotator's attention three ways
    over a screen that only ever had room for one. Recorded activity data is
    still read and preserved; there is simply no editor for it here.
    """
...
class TimelineWidget(QWidget):
    ...
    def mouseDoubleClickEvent(self, event: QMouseEvent) -> None:
        ...
        Error intervals win over the movement underneath them, because in error
        mode the two lanes overlap in intent: the user who double clicked an
        error band meant the error.
        """
        point = QPointF(event.position())
        if self._mode is TimelineMode.ERROR:
            hit = self._interval_at(point)
